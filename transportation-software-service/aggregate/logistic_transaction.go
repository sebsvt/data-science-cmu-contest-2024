...
type LogisticTransaction struct {
	TransactionID uuid.UUID     `bson:"transaction_id"` // Unique identifier for the transaction
	...
	PartnerID     uuid.UUID     `bson:"partner_id"`     // ID for the partner (SaaS identifier)
...
type LogisticTransactionRepository interface {
	FromTransactionID(ctx context.Context, transaction_id uuid.UUID) (*LogisticTransaction, error)
